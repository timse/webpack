...
function hasModule(chunk, module, checkedChunks) {
	...
	if(chunk.getParents().size === 0) return false;
	return allHaveModule(Array.from(chunk.getParents()).filter((c) => {
		...
		compiler.plugin("compilation", (compilation) => {
			compilation.plugin(["optimize-chunks-basic", "optimize-extracted-chunks-basic"], (chunks) => {
				for(var index = 0; index < chunks.length; index++) {
					...
					if(chunk.getParents().size === 0) continue;
					...
					for(var i = 0; i < modules.length; i++) {
						...
						if(dId === null) {
							parentChunksWithModule = allHaveModule(Array.from(chunk.getParents()), module);
						} else if(dId in cache) {
							parentChunksWithModule = cache[dId];
						} else {
							parentChunksWithModule = cache[dId] = allHaveModule(Array.from(chunk.getParents()), module);
						}
