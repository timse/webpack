...
const BinaryExpressionStaticEvaluator = {
...
const StringPrototypeCallExpressionEvaluator = {
...
const StaticEvaluationVisitor = {
	...
	"BinaryExpression": (expression, parser) => {
		...
		if(!BinaryExpressionStaticEvaluator.hasOwnProperty(normalizeOperator)) {
			return;
		}

		return BinaryExpressionStaticEvaluator[normalizeOperator](expression, parser);
	},
	...
	"CallExpression": (expression, parser) => {
		...
		if(StringPrototypeCallExpressionEvaluator.hasOwnProperty(property)) {
			return StringPrototypeCallExpressionEvaluator[property](expression, param, parser);
		}
...
class EvaluateTypeOfPlugin extends Tapable {
	constructor(parser) {
		super();
...
class EvaluateIdentifierPlugin extends Tapable {
	constructor(parser) {
		super();
		...
		parser.plugin("evaluate Identifier", (expr) => {
			...
			if(parser.scope.definitions.indexOf(expr.name) === -1) {
				var result = this.applyPluginsBailResult1("EvaluateIdentifierPlugin " + name, expr);
				if(result) return result;
				return new BasicEvaluatedExpression().setIdentifier(name).setRange(expr.range);
			} else {
				return this.applyPluginsBailResult1("EvaluateIdentifierPlugin defined " + name, expr);
			}
		});
		parser.plugin("evaluate MemberExpression", (expression) => {
			...
			if(expr.type === "Identifier") {
				...
				if(parser.scope.definitions.indexOf(name) === -1) {
					...
					exprName = exprName.join(".");
					if(parser.scope.definitions.indexOf(expr.name) === -1) {
						var result = this.applyPluginsBailResult1("EvaluateIdentifierPlugin " + exprName, expression);
						if(result) return result;
						return new BasicEvaluatedExpression().setIdentifier(exprName).setRange(expression.range);
					} else {
						return this.applyPluginsBailResult1("EvaluateIdentifierPlugin defined " + exprName, expression);
					}
				}
			}
		});
	...
	plugin(specifier, callback) {
		super.plugin("EvaluateIdentifierPlugin " + specifier, callback);
	}
}
...
class Parser extends Tapable {
	constructor(options) {
		...
		this.evaluateIdentifierPlugin = new EvaluateIdentifierPlugin(this);
		this.evaluateTypeOfPlugin = new EvaluateTypeOfPlugin(this);
		this.initializeEvaluating();
	}

	initializeEvaluating() {
		this.plugin("evaluate Literal", function(expr) {
			return StaticEvaluationVisitor[expr.type](expr);
		});
		this.plugin("evaluate LogicalExpression", function(expr) {
			return StaticEvaluationVisitor[expr.type](expr, this);
		});
		...
		this.evaluateTypeOfPlugin.plugin("evaluate typeof undefined", function(expr) {
		...
		this.plugin("evaluate TemplateLiteral", function(node) {
			return StaticEvaluationVisitor[node.type](node, this);
		});
