...
const CallExpressionPropertyVisitor = {
	"replace": (expression, param, parser) => {
		// we can only try to optimize if the param
		// on which `.replace` is called is a static string
	...
	"split": (expression, param, parser) => {
		// we can only try to optimize if the param
		// on which `.split` is called is a static string
		if(!param.isString()) return;
...
const EvaluationVisitor = {
	...
	"CallExpression": (expression, parser) => {
		...
		if(CallExpressionPropertyVisitor.hasOwnProperty(property)) {
			return CallExpressionPropertyVisitor[property](expression, param, parser);
		}
...
class Parser extends Tapable {
	...
	walkIdentifier(expression) {
		...
		const expressionName = this.scope.renames["$" + expression.name] || expression.name;
