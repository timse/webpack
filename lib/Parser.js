	...
	constructor(parser) {
		...
		parser.plugin("evaluate Identifier", (expr) => {
			...
			if(parser.scope.definitions.indexOf(expr.name) === -1) {
				var result = this.applyPluginsBailResult1(name, expr);
				if(result) return result;
				return new BasicEvaluatedExpression().setIdentifier(name).setRange(expr.range);
			} else {
				return this.applyPluginsBailResult1("defined " + name, expr);
			}
		});
		parser.plugin("evaluate MemberExpression", (expression) => {
			...
			if(expr.type === "Identifier") {
				...
				if(parser.scope.definitions.indexOf(name) === -1) {
					...
					if(parser.scope.definitions.indexOf(expr.name) === -1) {
						var result = this.applyPluginsBailResult1(exprName, expression);
						if(result) return result;
						return new BasicEvaluatedExpression().setIdentifier(exprName).setRange(expression.range);
					} else {
						return this.applyPluginsBailResult1("defined " + exprName, expression);
					}
				}
			}
		});
...
class Parser extends Tapable {
		...
		this.plugin("evaluate UnaryExpression", function(expr) {
			if(expr.operator === "typeof") {
				...
				if(expr.argument.type === "Identifier") {
					var name = this.nameInCurrentScope(expr.argument.name);
				...
				if(expr.argument.type === "MemberExpression") {
					...
					while(expression.type === "MemberExpression" && !expression.computed) {
						exprName.unshift(this.nameInCurrentScope(expression.property.name));
						expression = expression.object;
					}
					if(expression.type === "Identifier") {
						exprName.unshift(this.nameInCurrentScope(expression.name));
						if(this.scope.definitions.indexOf(name) === -1) {
							exprName = exprName.join(".");
	...
	walkWhileLoopStatement(statement) {
		this.walkExpression(statement.test);
		this.walkStatement(statement.body);
	}

	walkWhileStatement(statement) {
		this.walkWhileLoopStatement(statement);
	}
	...
	// Run a given function in a clone of the current scope
	inScope(params, fn) {
	...
	enterPattern(pattern, onIdent) {
		...
		const enterPatternType = "enter" + pattern.type;
		if(this[enterPatternType]) {
			this[enterPatternType](pattern, onIdent);
		}
	}
	...
	enterObjectPattern(pattern, onIdent) {
		...
		// Leave this a "var" for now.
		// The parser should be fast, and let/const sadly have performance issues.
		for(var propIndex = 0; propIndex < patternProperties.length; propIndex++) {
			this.enterPattern(patternProperties[propIndex].value, onIdent);
		}
	}
	...
	enterArrayPattern(pattern, onIdent) {
		...
		// Leave this a "var" for now.
		// The parser should be fast, and let/const sadly have performance issues.
		for(var elementIndex = 0; elementIndex < patternElements.length; elementIndex++) {
			this.enterPattern(patternElements[elementIndex], onIdent);
		}
	}
	...
	parseStringArray(expression) {
		...
		// if it doesnt contain elements return an empty array
		if(!expression.elements) {
			return [];
		}

		// apply each child to parseString
		return expression.elements.map(expr => this.parseString(expr));
	}
	...
	parseCalculatedStringArray(expression) {
		...
		// apply each child to parseCalculatedString
		return expression.elements.map(expr => this.parseCalculatedString(expr));
	}
