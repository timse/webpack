	...
	parser.plugin("call define", function(expr) {
		...
		if(fn && fn.type === "FunctionExpression") {
			...
			this.inScope(fnParams, function() {
				...
				this.traverseNodeInCurrentScope(fn.body);
			}.bind(this));
			...
			this.inScope(fn.callee.object.params.filter(function(i) {
				...
				this.scope.renames = fnRenames;
				this.scope.inTry = inTry;
				this.traverseNodeInCurrentScope(fn.callee.object.body);
			}.bind(this));
			if(fn.arguments)
				this.traverseNodesInCurrentScope(fn.arguments);
		} else if(fn || obj) {
			this.traverseNodeInCurrentScope(fn || obj);
		}
